...
class JSONReporter {
    ...
    specStarted(spec) {
       // this.JSONObj[suite].push('Spec started: ' + spec.description + '\nFull description: ' + spec.fullName);
    }

    specDone(spec) {
        var suite = spec.fullName.replace(spec.description, '').trim();     
        this.JSONObj[suite].push('Spec: ' + spec.description + ' was ' + spec.status);

        for(var i = 0; i < spec.failedExpectations.length; i++) {
            this.JSONObj[suite].push('Failure: ' + spec.failedExpectations[i].message);
          }

        this.JSONObj[suite].push(spec.passedExpectations.length);

       // this.JSONObj[suite].push(spec.failedExpectations);
    }

    suiteDone(suite){
        //something to be put at the end of suite execution
    }
